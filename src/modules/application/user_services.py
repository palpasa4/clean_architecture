...
class UserService():
    ...
    def check_duplicate_user(self, username:str) -> None:
        try:
            ...
            raise UserDatabaseOperationError(message="Database error while checking for duplicate user", status_code=500)
    ...
    def create_user_handler(self, user_entity: User) -> User:
        try:
            ...
            logger.error(f"User creation failed for username='{user_entity.username}': {str(e)}")
            raise UserDatabaseOperationError(message="Unable to create user.", status_code=500)
    ...
    def check_valid_user(self, model: UserLoginModel) -> User:
        try: 
            ...
            raise UserDatabaseOperationError(message="Database error while validating user.",status_code=500)
    ...
    def user_view_details(self, id) -> UserViewDetails:
        try:
            ...
            logger.error(f"[User Access] Database error while retrieving user details. Error: {str(e)}")
            raise UserDatabaseOperationError(message="Database error while retrieving user details.",status_code=500)
    ...
    def user_view_transactions(self, id: str)-> list[UserTransactionDetails]:
        try: 
            ...
            logger.error(f"[User Access] Database error while retrieving transaction details. Error: {str(e)}")
            raise UserDatabaseOperationError(message="Database error while retrieving transaction details.",status_code=500)
